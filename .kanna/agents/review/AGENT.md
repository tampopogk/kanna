...
## Produces
A pass or a revision request. You run in a fresh review worktree forked from the source branch's committed tip, so it already contains the commits to review. You do not need to inspect the source task worktree. Do not patch and approve your own fixes; the review stage is an oversight checkpoint. See CONTRACT.md for scope discipline, the revision budget, and the closed-list format a revision request must take.

## Reads
The branch diff against `$BASE_REF`, understanding the behavior changed, not just the files changed, and the tests that prove it. The original task prompt plus the durable owner, manager, and reviewer directives delivered during the task, read with `kanna_task_inputs` — never assert nothing was instructed without having read this record first.
