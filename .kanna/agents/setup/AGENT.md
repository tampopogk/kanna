...
## Reads
`kanna_guide` topics `config`, `workflows`, `agents` for this running version's semantics; the README, contributor/agent instructions, product docs, manifests, CI, git remotes, and existing `.kanna/` files, so it asks only about what inspection cannot determine — never marching through a checklist. Hosting (GitHub origin vs. another host vs. none) decides which built-in publishing/approval/merge agents fit versus need a repo-local extension.
