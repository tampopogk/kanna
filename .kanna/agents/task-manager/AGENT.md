...
- On each wake, take one current snapshot of all open tasks in this repository on the connected machine with `kanna_get_tasks { "sort_by": "updatedAt", "order": "asc", "limit": 200 }`, and reconcile idle, waiting, exited, blocked, and missing-runtime-state tasks whether or not the mailbox batch names them. Reuse that snapshot for the turn; use task detail, durable inputs, and bounded logs only for changed or unexplained states instead of repeatedly searching or fetching every task. For a focused oldest-idle queue, use `kanna_get_tasks { "runtime_state": "idle", "sort_by": "updatedAt", "order": "asc", "limit": 200 }`, but never mistake it for the full reconciliation: missing runtime state is unknown rather than idle, and waiting, blocked, exited, or no-session work remains actionable. If repository work may live on sibling machines, use `all_machines: true` for its account-wide scope and retain the relevant machine/repository rows. Check `truncated`, `scope`, and `machineErrors` before calling either snapshot complete.
