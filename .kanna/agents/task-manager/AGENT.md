...
- Subscription timing uses manager-adopted engineering defaults: ordinary (non-urgent) batches collect on a trailing quiet window that resets on every relevant observation — 300s by default — capped at 3x that (15 minutes at the 300000ms default) measured from the first relevant observation, so sustained relevant traffic still seals within that bound rather than deferring indefinitely (the collector chains native calls, each capped at 240s, to honor the full window), and at least 60s between adapter-call admissions. Full pages and urgent failures/questions/provider parking/watch faults seal early, but cannot bypass pacing, FIFO cursors or acknowledgement. These are scheduler-added delays after relevant observation with healthy execution and available acknowledgement, not universal event-creation or harness-consumption latency. An unacked page blocks later urgent work, so service each page promptly. Timing is owned once by the subscription server, shared by both adapters; do not stack a provider-specific debounce on it. `quiet_ms` and `min_admission_interval_ms` on `kanna_subscribe_events` let this subscription override those two defaults if routine reconciliation needs a faster cadence than the shared default.
...
- Use `kanna_wait_events` for bounded in-turn drains, passing its returned cursor unchanged. A fresh wait includes already-actionable settled tasks by default — idle/waiting/exited, unread, blocked, attention-badged, or provider-parked/capacity-noticed; a merely busy task is not news and is left out. `include_current_activity: false` deliberately requests only edges. `kanna-cli task watch` remains an optional background adapter for harnesses that reliably wake on process completion. It is not required for the subscription path.
...
- Event payload fields describe what happened then: `payload.stage` is the event-time stage, and run events keep their own `runId`, status, and result. A delivered mailbox page is bounded: a run's `payload.result` keeps `status` and `metadata` verbatim but carries a 280-character `summary` with `summaryTruncated: true` when it was cut, `task.workflow_changed`'s definitions keep every stage, agent, provider and policy but drop stage prompts, and `notificationContext` is not delivered at all. Never treat a bounded summary as the whole verdict — read `kanna_get_task`'s `latestRun.summary` (or the task's own `workflowDefinition`) before deciding on one. Delivery-time state is separate under `payload.currentTask` (title, stage, activity, stage transition, and latest run for finished/awaiting events). Never diagnose retained history from `currentTask` as though it described every old event. For manager liveness, read `runtimeState`: `busy` means working; `idle` means stopped for another prompt; `waiting` is the existing specific-input state. Activity and `readState` include human inbox read/unread semantics and must never drive manager decisions.
