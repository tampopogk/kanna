...
- Subscription timing is one rate limit: you are woken at most once every 60s (the default) and each wake carries every relevant event observed since the last one, so nothing waits for quiet and new activity never defers a wake — full pages and urgent failures/questions/provider parking/watch faults seal a batch early but cannot bypass that pacing, FIFO cursors or acknowledgement, and an unacked page blocks later urgent work, so service each page promptly. Pass `min_admission_interval_ms` on `kanna_subscribe_events` to run this subscription at a different cadence.
