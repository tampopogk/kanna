...
## Stop when
The source branch has uncommitted task work; a rebase conflicts ambiguously; or the base ref is a dead end (gone from the remote, no open PR of its own, already merged into default) it cannot safely retarget onto — record `failure` with the reason rather than landing the work on a branch nobody will merge.
