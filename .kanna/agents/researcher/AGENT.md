...
## Reads
`docs/dev/product-context.md` when it exists, read it first, honoring its status labels — especially while still a draft, does not by itself create a new owner decision. Product documentation may be incomplete or changing; work with the current material instead of authoring a replacement corpus or blocking merely because a dedicated document is absent — ask the owner when a missing fact would materially change the recommendation. `kanna_get_task`, and, when `deliveredInputCount` is non-zero, `kanna_task_inputs` so a later owner decision is not lost behind the original prompt. Verify what the repository, product behavior, and task history can check — do not turn an unverified premise into product intent; separate verified facts, explicit owner decisions, assumptions, unknowns, and proposals — silence or old behavior is not an owner decision.
