...
- **Recut an unreleased series:** `./kd release cut --version <X.Y.0> --recut --confirm-recut <active-staging-version|empty> --confirm-old-tip <old-branch-sha> --reason "<why>"`, then ship the RC with `./kd release ship --staging --release --branch release/<X.Y>`. Use this only to move that stabilization branch onto current `origin/main` so a branch RC carries newer work; it is not required to keep the ordinary main train moving or to promote a retained RC. Read both confirmations from `./kd release status` and `git ls-remote origin refs/heads/release/<X.Y>` immediately before the command and never guess them; rehearse with `--dry-run` first.
...
- **Rehearse:** use `--dry-run` instead of `--release`; a dry-run builds and signs but does not notarize or publish, and it runs the same identity, lineage, version, and soak gates as the real operation.
...
`kd` enforces the channel's lineage, so read a refusal as information rather than an obstacle. A staging publish must descend from the candidate the channel already serves unless a narrow recorded transition authorizes otherwise; divergence, rollback, and unreadable channel metadata are refused before any build. A newer publish does not erase an older immutable RC's own eligibility. Report a refusal and stop; do not work around it with a rollback, a reset, or a different branch unless that exact operation was authorized.

`./kd release status [--candidate X.Y.Z-staging.N]` separates the live staging pointer from the candidate being assessed. Never describe a candidate as promotable or ready from immutable identity alone — report `promotion.allowed` and, when it is false, every entry in `promotion.blockers`, including lineage validity, that selected RC's own soak (`promotion.soak`), forward production version, and an abandoned series.

Promotion is production: it must rebuild the exact selected RC commit with production identity and refuses unless HEAD, the immutable tag/release/manifest identity, historical lineage, series state, the selected RC's own soak, clean tree, and forward unused `vX.Y.Z` all pass. Main, the source branch, and `desktop-staging` may have advanced; promotion publishes the release tag without rewinding any of them. After branch promotion, report that the branch may go dormant for future patch backports. The production-version floor lets forward main start the next minor RC while trunk's `VERSION` is stale. See `docs/specs/release-candidates.md`.
