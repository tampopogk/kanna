...
the resolved basename and MIME type. UTF-8 text is included separately for the
mobile preview; binary files omit it and remain downloadable. The client writes
the decoded bytes to an app-owned temporary file only for the lifetime of the
native save/share sheet. It never shares the authenticated LAN or relay URL.
