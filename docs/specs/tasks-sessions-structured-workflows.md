...
| **Exit** | A named way out of a stage, declared by the workflow and mapped to a destination: `advance` (the next stage) or a loop back to a named earlier stage. A session may name the exit it recommends in its result; it cannot name a stage. **Owner.** |
...
| **Result** | What a session records at the end of its work: a status (did the session do its job), a message, optionally the exit it recommends, optionally artifacts. The engine attaches the committed sha and the session reference. |
...
| **Workflow** | A named, reusable stage chain with its transitions: which stage follows which, which transitions are manual, which exits each stage offers and where they lead, budgets, prompts, setup and teardown. Two families: *intake workflows* for work coming in (mechanical, shaped, planned, designed, research, specialized-reviewers) and *release workflows* for work going out (merge window, QA gauntlet, staging, soak, production). **Owner.** A task's pinned workflow is the definition it was created from plus any later plan replacement. |
...
|---|---|
| A task has stages; every session ends by recording one result; every transition is recorded with its operator; a session may choose only among the exits its stage declares, and an exit never operates a manual gate | Which stages a workflow has; which exits each stage offers and where they lead; which transitions are manual; loop budgets |
...
- Stage fields (from the workflow): `name`; `role` (agent definition, optional); `prompt`; `transition: manual | auto` (how the task leaves this stage through its `advance` exit); `exits` (named exits, see below); `loop_transition: manual | auto` (how the task leaves this stage when it was re-entered by a loop; default the same as `transition`); `budget` (agent-caused loops back into this stage, default 5); `exit_commit: true` (the commit step, below); `setup` and `teardown` scripts. The engine delivers the previous result and the ledger to every session in its environment; workflows do not need prompt variables for them. **Owner:** no `$RESULT`-style variables.
- **Status says whether the session did its job; the exit says where the task goes; the workflow bounds both.** **Owner.** A review that fails the branch has succeeded at reviewing: its status is `success`, its findings are its message, and its exit says where the findings go. A session records its result at the end of its work; that completes the session, not the stage. The stage exits only on a transition, and the transition is decided as follows:
  - **Exits are declared by the workflow, by name.** Every stage has `advance`, mapped to the next stage. A stage may declare loop exits mapped to earlier stages, for example `exits: {revise: "in progress", replan: "plan"}`. A session may name one of its stage's exits in its result. It cannot name a stage, so it cannot invent a route, and the workflow file shows every path a task can take.
  - **Default when no exit is named.** Status `success` takes `advance`; any other status parks the task in the stage with the message visible. Most agents never choose; only reviewers and dispatchers do, and only among two or three names.
  - **`advance` obeys the stage's transition policy.** With `transition: auto` the task moves to the next stage once every dependency edge into it is satisfied. With `transition: manual` the task parks at the gate until a person or a manager advances it, and the operator is recorded. Naming `advance` never operates a gate: an agent gets no human authority by choosing an exit. Until a manual gate is operated the person may keep working with the session, send it more input, and have it record again; the latest result is what the next stage receives.
  - **A loop exit is bounded by the destination's budget.** Taking `revise` or `replan` moves the task back to that stage, carries the result message with it, and spends one unit of the destination stage's budget. When the budget is spent the result is still recorded and the task parks; a person or manager may advance it back anyway, which resets the count.
  - **Statuses other than `success`** (`unverified`, `partial`, `needs-input`, `declined`, `failure`) mean the session could not fully do its job; the task parks with the message, whatever exit was named. `needs-input` is for a decision only a person can make.
  - **The chosen exit is provenance.** The ledger's transition entry records which exit was taken, whether the session named it or the default applied, so a reviewer that sends everything back to plan is visible and correctable.
- This is today's implement/review loop with two changes: the reviewer chooses among exits the workflow declares instead of naming a target stage, and the review's status no longer doubles as the routing signal. Verified 2026-09-22: today the review agent calls a revision request naming the target stage; the engine claims a budget round, starts a new session of the target stage, resumes the previous conversation in its old worktree when the tip matches, and the re-entered stage exits automatically on success. On this machine 153 agent-requested loops ran across single-reviewer and plan-build-review tasks, every one to `in progress` and none to `plan`, though the review agent's definition hard-codes `in progress` so the data does not show whether a plan exit would be used when offered. Tasks that looped reached a PR at a higher rate than tasks that never did. The budget, the resume, and the automatic exit after a loop are kept. **Owner.**
- Loops: a transition back to an earlier stage, whether through a loop exit or a person or manager advancing backwards, starts a new session of that stage in that stage's workspace (§6). The stage exits afterwards per `loop_transition`, so an autonomous implement/review loop needs no ⌘S between rounds. **Owner:** the two loop shapes are iterate within a stage, and go back to an earlier stage and come forward again (mockup → stakeholder review → mockup → stakeholder review); the autonomous implement/review loop is typically three to five rounds.
...
- **Commit step of a transition.** **Owner.** ⌘S on an implementation stage means "commit, then advance." A stage may declare `exit_commit: true`. When its transition is requested (manual advance or auto), the engine sends the live session one instruction, commit your work and record your result, in the same session and workspace so nothing is lost; the transition fires on that result. If the session is dead, a short commit session runs in the same workspace instead. This is the only mechanism by which uncommitted work reaches the boundary; it is part of the transition, not a stage.
...
- **Name**: stage-scoped, set by the stage definition from the task title and stage (for example `Implement: <title>`); the agent may rename its session. Task lists show task titles; session views show session names. **Owner.**
...
- Ledger storage: `~/.kanna/repos/<repo-id>/tasks/<task-id>/` holding `task.json` (prompt, workflow, links) and an ordered `ledger/` of entries: `NNNN-result.md`, `NNNN-transition.json` (from, to, exit taken, operator, declared role, verified channel), `NNNN-plan.json` (replacement, source), `NNNN-input.md` (tool-delivered input with provenance; terminal typing is in the transcript, not here). **Owner:** the ledger is internal message passing between stages and lives only here; nobody outside the machine reads it. The real outputs are commits, PRs and artifacts.
- Delivery to the next session: the engine puts the result that caused the transition into this session (whichever stage recorded it, so an implement session re-entered after a review's `revise` exit receives the review's findings) and the ledger path into the session's environment preamble. Sessions that need an earlier result (the plan, say) read it from the ledger. No prompt variables carry results. **Owner.**
...
- A workflow is a JSON document: `name`, `description` (a context sentence saying when to use it), `budget` default, `stages[]` with the fields in §5. Workflow definitions live in the bundled defaults and in repo `.kanna/workflows/`, with `config.json`/`config.local.json` choosing which applies.
...
- **Intake workflows**, by pattern of work (descriptions are context sentences; stage roles in order; `M` a manual transition out of the preceding stage, `A` automatic):
  - **mechanical**: implement → pr(M). The test suite is the reviewer.
  - **shaped**: implement → review(A) → pr(M); the review stage declares `exits: {revise: "implement"}`; loop under budget.
  - **planned**: plan → implement(M) → review(A) → pr(M); the review stage declares `exits: {revise: "implement", replan: "plan"}` and chooses by the nature of its findings.
...
- **Release workflows** (**Owner:** the shape is decided; the stages and their policies are the repository's). A release workflow runs as a task like any other, usually long-lived per repository, and its stages use the same schema:
...
- `advance {task, to?, source}` — a person or manager operates a manual transition, forward or back; records operator provenance. Agents do not call it.
- `record_result {task, status, message, exit?, artifacts?}` — ends the session's work; `exit` names one of the stage's declared exits; the engine attaches sha and session, then applies the workflow's transition rule (§5); may carry `remaining_plan` for a plan stage. There is no separate revision-request call.
...
5. Workflow schema (transition, exits, loop_transition, budget, exit_commit, agent-less stages, setup/teardown), plan replacement with the single validation rule, the intake lineup. Done when the implement/review loop runs with the reviewer naming an exit and never a stage, and a review whose findings are structural reaches plan through `replan`.
...
10. Release workflow: the schema fits merge window, QA gauntlet, staging, soak and production; V1 keeps the existing merge handoff. Done when the existing merge master runs as the merge-window stage of a release-workflow task.
...
- Review loops: status says whether the session did its job; the workflow declares each stage's exits by name; a session may choose among them and never names a stage; `advance` never operates a manual gate; a loop exit spends the destination's budget; the chosen exit is recorded. Today's budget, resume and automatic exit after a loop are kept.
- Stages without an agent are allowed; their setup and teardown scripts do the work (mail the reviewers, open the window).
- No prompt variables for results; the engine delivers the triggering result and the ledger to every session. **Owner** (2026-09-23): the target lineup and the converted agent definitions use none; the bundled legacy workflows kept for compatibility (no-review, single-reviewer, plan-build-review) still do. Result variables keep working as a documented, deprecated compatibility feature rather than being removed, since a user's definition is theirs.
- Disk authority (2026-09-23, **Owner**): ships with SQLite as the default authority and disk as an explicit opt-in (`kanna-server storage-authority disk`); making disk the default is a later rollout after it has run on staging.
