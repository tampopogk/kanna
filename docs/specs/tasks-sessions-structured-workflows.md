...
- No prompt variables for results; the engine delivers the triggering result and the ledger to every session. **Owner** (2026-09-23): Kanna's shipped definitions use none; existing result variables in user-authored definitions keep working as a documented, deprecated compatibility feature rather than being removed, since a user's definition is theirs.
