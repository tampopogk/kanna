...
impl<'alloc: 'cb, 'cb> Terminal<'alloc, 'cb> {
    ...
    ///
    /// Unlike [`Terminal::grid_ref`], this reads the requested screen even
    /// when it is not the active one. Returns `Ok(None)` if the requested
    /// screen does not exist yet, for example when the alternate screen has
    /// never been entered.
    pub fn screen_grid_ref(&self, screen: Screen, point: Point) -> Result<Option<GridRef<'_>>> {
        let mut grid_ref = ffi::sized!(ffi::GridRef);
        let result = unsafe {
        ...
        Ok(
            from_optional_result(result, grid_ref)?
                .map(|inner| unsafe { GridRef::from_raw(inner) }),
        )
    }
    ...
    ///
    /// Returns `Ok(None)` if the screen does not exist yet.
    pub fn screen_scrollback_rows(&self, screen: Screen) -> Result<Option<usize>> {
        self.screen_get_optional(screen, Data::SCROLLBACK_ROWS)
    }
    /// The total number of rows in a specific screen including scrollback.
    ///
    /// Returns `Ok(None)` if the screen does not exist yet.
    pub fn screen_total_rows(&self, screen: Screen) -> Result<Option<usize>> {
        self.screen_get_optional(screen, Data::TOTAL_ROWS)
    }
    /// Get the cursor column position (0-indexed) for a specific screen.
    ///
    /// Returns `Ok(None)` if the screen does not exist yet.
    pub fn screen_cursor_x(&self, screen: Screen) -> Result<Option<u16>> {
        self.screen_get_optional(screen, Data::CURSOR_X)
    }
    /// Get the cursor row position (0-indexed) for a specific screen.
    ///
    /// Returns `Ok(None)` if the screen does not exist yet.
    pub fn screen_cursor_y(&self, screen: Screen) -> Result<Option<u16>> {
        self.screen_get_optional(screen, Data::CURSOR_Y)
    }
    /// Get whether a specific screen's cursor has a pending wrap.
    ///
    /// Returns `Ok(None)` if the screen does not exist yet.
    pub fn screen_is_cursor_pending_wrap(&self, screen: Screen) -> Result<Option<bool>> {
        self.screen_get_optional(screen, Data::CURSOR_PENDING_WRAP)
    }
    /// Get the current SGR cursor style for a specific screen.
    ///
    /// Returns `Ok(None)` if the screen does not exist yet.
    pub fn screen_cursor_style(&self, screen: Screen) -> Result<Option<style::Style>> {
        self.screen_get_optional::<ffi::Style>(screen, Data::CURSOR_STYLE)?
            .map(std::convert::TryInto::try_into)
            .transpose()
    }
    /// Get the Kitty keyboard protocol flags for a specific screen.
    ///
    /// Returns `Ok(None)` if the screen does not exist yet.
    pub fn screen_kitty_keyboard_flags(
        ...
        Ok(self
            .screen_get_optional::<ffi::KittyKeyFlags>(screen, Data::KITTY_KEYBOARD_FLAGS)?
            .map(key::KittyKeyFlags::from_bits_retain))
    }
    /// Get the scrollbar state for a specific screen's viewport.
    ///
    /// Returns `Ok(None)` if the screen does not exist yet.
    pub fn screen_scrollbar(&self, screen: Screen) -> Result<Option<Scrollbar>> {
...
mod tests {
    ...
    fn screen_grid_ref_reads_inactive_primary_and_active_alternate() {
        ...
        assert_eq!(terminal.screen_cursor_x(Screen::Primary).unwrap(), Some(2));
        assert_eq!(
            terminal.screen_cursor_x(Screen::Alternate).unwrap(),
            Some(2)
        );
    }
    ...
    fn screen_specific_getters_return_none_for_missing_alternate() {
        ...
        assert_eq!(terminal.screen_cursor_x(Screen::Alternate).unwrap(), None);
        assert_eq!(terminal.screen_total_rows(Screen::Alternate).unwrap(), None);
        assert!(
            terminal
                .screen_grid_ref(
                    Screen::Alternate,
        ...
        assert_eq!(
            terminal.screen_total_rows(Screen::Primary).unwrap(),
            Some(2)
        );
    }
