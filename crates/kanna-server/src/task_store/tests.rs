...
fn live_stage_changes_record_their_trigger_and_never_borrow_one() {
    ...
    fixture
        .db
        .update_pipeline_item_stage_with_trigger("task-1", "review", crate::db::StageTrigger::Auto)
        .unwrap();
    // Manual advance with no result since the previous transition.
    fixture
        .db
        .update_pipeline_item_stage_with_trigger("task-1", "pr", crate::db::StageTrigger::Operator)
        .unwrap();
