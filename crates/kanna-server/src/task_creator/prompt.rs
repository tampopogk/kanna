...
pub(super) fn build_revision_resume_message(
    ...
    let completion = match transition {
        ...
        WorkflowStageTransition::Manual => format!(
            "Address the feedback in this worktree, then finish with a clear summary of what you changed — do not record stage completion; this stage advances manually, so the user reviews the revision and advances the task themselves. If you cannot address the feedback, record the status that fits instead of stopping silently — `needs-input` when the feedback does not say enough to act on, `declined` when a finding is wrong or already fixed, `partial` when you addressed some of it, `unverified` when you changed it but could not prove it, `failure` when you tried and could not: call MCP `kanna_complete_stage {{\"task_id\": \"{task_id}\", \"status\": \"...\", \"summary\": \"...\"}}`; only if MCP tools are unavailable, fall back to `kanna-cli stage-complete --task-id \"{task_id}\" --status ... --summary \"...\"`."
        ),
    };
    format!(
        "{}\n\n{completion}",
