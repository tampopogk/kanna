...
const EVENT_SUPERVISION_GUIDANCE: [&str; 6] = [
    ...
    "`task.activity_changed` is the human read/unread display dimension, fired on the blended `activity` value — not on the runtime dimension — so a person merely reading a task's output moves it. Managers should drop it with `--exclude-event-type task.activity_changed` / `exclude_event_types` and watch `task.runtime_changed` instead; `kanna-cli task watch` already suppresses it. Confirm the current task with `kanna_get_task` and read `runtimeState`, then inspect `waitingPromptSnippet` when present; a display transition is not proof that the snippet is a question. For manager wakeups, use `task subscribe-events --task-id <manager-id> --repo-id <repo-id>` once. Kanna watches outside the model and sends a supervisory mailbox nudge; read with `task read-event-subscription --subscription-id <id>` and acknowledge the serviced batch with `--acknowledge-batch-id`. Harnesses with reliable background completion may still use `task watch`. A cursorless wait skips retained history by default (`from` defaults to `now`, not full replay) and instead returns the current actionable snapshot — idle, waiting, exited, unread, blocked, attention-badged, or provider-parked/capacity-noticed tasks, never a merely busy one; pass `from=beginning` to explicitly replay retained history too. Continuation cursors acknowledge that scan without changing human read state. Run from inside a task session (`KANNA_TASK_ID` set), a repository- or parent-default-scoped watch or `kanna_wait_events` call excludes the calling task's own events by default — a task-scope filter computed client-side into `excludeTaskIds`, so your own turn ending busy → idle never wakes you; pass `--exclude-own=false` / `exclude_own: false` to observe your own events, or `--exclude-task-id` / `exclude_task_ids` to silence other tasks. This never suppresses an event on some OTHER watched task merely because your own action caused it — waking on a watched child's own `task.input_delivered` echo after you sent it input is expected and correct. Exclusions are a filter, not a scope: a cursor survives changing them, whereas switching between `repo_id`, `task_ids`, and `parent_task_id` scopes is rejected with `cursor belongs to a different task-event scope` and must restart at the live tail (`from: now`). The CLI owns cursor advancement and can loop for an arbitrary budget; `kanna_wait_events` stays clamped to 240 seconds because MCP clients commonly abort around 300 seconds and lose the result. Make each call carry more instead: `event_types` names the short list you act on rather than excluding every noisy type, `min_events` and `debounce_ms` batch a burst into one response, and `min_interval_ms` rate-limits the wake itself. Several events for the same task in one response collapse into one current-state row carrying `causedByEventTypes` instead of the whole transcript — state, not a transcript, for a burst. Batching changes only how often you wake, never which events you are given.",
